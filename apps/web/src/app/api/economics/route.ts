...
import {createExposureRepository} from '../../../../../../packages/metrics/exposure.mjs';
...
async function handle(request:NextRequest){let finish=(r:NextResponse)=>r;try{
 ...
 const auth=requestAuth(request);finish=auth.finish;const database=createDatabase({identity:identity(auth.client),pool:serverPool(),selectedTenant:request.cookies.get(ACTIVE_ORG)?.value});const repository=createEconomicRepository({database});const exposure=createExposureRepository({database});let data;
 ...
 }else{if(request.nextUrl.search)throw new AccessError(400,'unsupported_filter');const raw=await request.text();if(Buffer.byteLength(raw)>32768)throw new AccessError(400,'input_too_large');let parsed;try{parsed=JSON.parse(raw);}catch{throw new AccessError(400,'input_invalid');}if(!parsed||Array.isArray(parsed)||typeof parsed!=='object')throw new AccessError(400,'input_invalid');const {operation,...input}=parsed;if(operation==='source')data=await repository.recordSource(input);else if(operation==='record')data=await repository.record(input);else if(operation==='link')data=await exposure.record(input);else if(operation==='orderAlias')data=await exposure.recordAlias(input);else if(operation==='orderCustomer')data=await exposure.recordCustomer(input);else if(operation==='relationCoverage')data=await exposure.recordCoverage(input);else throw new AccessError(400,'operation_invalid');}
