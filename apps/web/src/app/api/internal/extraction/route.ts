...
export const POST=createExtractionHostedHandler({secret:process.env.VEXA_WORKER_TRIGGER_SECRET,runtime:async()=>{
 ...
 try{return {...worker,extraction:createExtractionRuntime({database:worker.database,storage:worker.storage,resolveConfig:createExtractionConfigResolver(process.env.VEXA_EXTRACTION_CONFIG_JSON??'[]'),env:process.env,runtime:process.env.VEXA_AI_RUNTIME,apiKey:process.env.OPENROUTER_API_KEY})};}
