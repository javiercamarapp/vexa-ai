...
import {createHistoryRuntime} from '../../../../../../../packages/history/runtime.mjs';
...
export async function POST(request:Request){
 ...
 try{
  ...
  const problems=createHistoryRuntime({database:worker.database,resolveConfig:historyConfig()});
  return Response.json({data:await problems.tick()},{headers:{'Cache-Control':'private, no-store'}});
 }catch{return reply('HISTORY_WORKER_UNAVAILABLE',503);}
