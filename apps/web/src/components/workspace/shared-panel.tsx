  ...
  {bundle&&!busy&&<>
   ...
   <DataState state={state==='empty'?{kind:'empty'}:{kind:state??'partial',coverage:bundle.meta.coverage,watermark:bundle.meta.watermark}}>
    <div className="record-grid">{bundle.items.map(item=><article key={item.id} className="record-card"><header><span className="status">{item.status}</span><h2>{resource==='problems'?<Link href={'/problems/'+item.id+'?'+pinned.toString()}>{item.title}</Link>:item.title}</h2></header><p>{item.summary}</p>
     <dl className="metric-grid">{item.metrics.map((metric,index)=><div key={index}><dt>{metric.label}</dt><dd>{formatMinorUnits(metric.amount_minor,metric.currency,metric.exponent)}</dd><small>{metric.kind} · Referencia: {metric.source_ref}</small>{metric.amount_minor===null&&metric.known_subtotal!==null&&<p>Subtotal conocido: {formatMinorUnits(metric.known_subtotal,metric.currency,metric.exponent)}; no es el total.</p>}</div>)}</dl>
