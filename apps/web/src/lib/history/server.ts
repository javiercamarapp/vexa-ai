...
export async function history(request:NextRequest){let finish=(r:NextResponse)=>r;const headers={...PRIVATE_HEADERS,Vary:'Cookie'};try{
 ...
 const auth=requestAuth(request);finish=auth.finish;const database=createDatabase({identity:identity(auth.client),pool:serverPool(),selectedTenant:request.cookies.get(ACTIVE_ORG)?.value}),repository=createHistoryRepository({database,resolveConfig:historyConfig(process.env,database)});
