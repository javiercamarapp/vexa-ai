...
export async function handleTeam(request:NextRequest,invitationId?:string){let finish=(r:NextResponse)=>r;try{const c=config();if(!c)throw new AccessError(503,'team_configuration_required');if(request.method==='POST')assertOrigin(request.headers.get('origin'),c.origin);const auth=requestAuth(request);const commitCookies=(r:NextResponse)=>{const response=auth.finish(r);response.headers.set('Referrer-Policy','no-referrer');return response;};const input=request.method==='POST'?await body(request):null;const sessionExchange=Boolean(invitationId&&input?.operation==='session');if(!sessionExchange)finish=commitCookies;
