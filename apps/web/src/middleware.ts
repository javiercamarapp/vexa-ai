...
function authLinkHeaders(request:NextRequest,response:NextResponse) {
  if (/^\/auth\/(?:invitations|email)(?:\/|$)/.test(request.nextUrl.pathname)) response.headers.set('Referrer-Policy','no-referrer');
  return response;
}
