...
## Recuperación 296: rutas ejecutivas CSR y dependencia no configurada

Reproducción sobre producto publicado `9367460`, anterior a Notificaciones: `/overview` entrega shell autenticada y después consulta `/api/workspace`. La comprobación inmediata del número de alertas fallaba durante carga. El control ahora espera la respuesta de la API exacta correspondiente y la alerta accionable; conserva ocho rutas, cuatro roles reales, sesión inválida, selector de organización ajeno, ausencia de datos ajenos y las comprobaciones de teclado/estados existentes.

Para las seis rutas ejecutivas sin pins, la dependencia es `/api/workspace` (`f06-workspace-v1`, `workspace_request_failed`). Los dos detalles financieros reciben pins sintéticos explícitos para activar su petición `/api/workspace/detail/{problem|customer}/{id}` (`f06-detail-v1`, `detail_request_failed`): esos pins no representan una publicación real ni prueban autorización sobre datos financieros. Se exige 503, JSON privado/no-store, error reintentable, UUID de traza y ausencia de datos. No se acepta una lista genérica de errores 503.

Algunos paneles descartan el cuerpo de error y `response.json()` de Playwright quedó pendiente en `/recommendations`. Se preservó su stack. El control observa ruta, status y headers de la respuesta original del navegador; un segundo GET real, con las mismas cookies y URL exacta y timeout de 10 segundos, comprueba el contrato actual completo. No se afirma que ese segundo cuerpo sea el de la primera respuesta. La alerta visible y la ausencia de estado vacío/listo se comprueban separadamente en el DOM.

Límite explícito: con la base de datos de workspace no configurada, las queries inválidas y `tenant_id` forjado sólo acreditan fallo cerrado por dependencia, sin datos. No acreditan un 400 ni la validación productiva de esos parámetros. Los errores específicos de `parseScope` se comprueban en la ruta de examen real y la autorización de datos con base configurada sigue cubierta por los gates F06 correspondientes. La cookie de organización ajena sí debe denegarse mediante Auth real antes de mostrar shell.

Calibración preservada: sustituir el estado de error por «Sin resultados para este alcance» en una copia temporal del componente produce `DEPENDENCY_VISIBLE`; restaurar sus bytes recupera la comprobación. Los negativos unitarios rechazan status, contrato, caché, identidad, traza y presentación incorrectos. No se modifica producto ni se vuelve a ejecutar F01-02 o la matriz SQL.
