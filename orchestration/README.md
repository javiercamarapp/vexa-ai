...
Auth tiene timeout ≤15 s. Cada examen (verify, run y materialización limpia de accept) usa el menor entre `turn_timeout_seconds` del grafo y el presupuesto restante de la invocación; el supervisor conserva además su deadline global. No hay un tope oculto adicional de 300 s que corte un examen SQL/Auth/Storage válido. `--max-minutes 1` sigue limitando la etapa a un minuto aunque el grafo permita 900 s. Operaciones Git tienen timeout individual de 30 s; I/O y cleanup agregan overhead, no se promete finalización exacta al milisegundo del deadline. Timeout limpia miembros supervivientes del mismo process group aunque líder ya haya terminado; procesos deliberadamente desligados requieren aislamiento/supervisión de host y están fuera del modelo cooperativo.
