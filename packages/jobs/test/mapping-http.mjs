...
try {
 ...
 dock(['cp',path.join(root,'packages/jobs/test/mapping-browser.cjs'),browserName+':/tmp/browser.cjs']);
 const childBrowser=spawn('docker',['exec','-i',browserName,'node','/tmp/browser.cjs'],{stdio:['pipe','pipe','pipe']});let output='';childBrowser.stdout.on('data',x=>output+=x);childBrowser.stderr.on('data',x=>output+=x);childBrowser.stdin.end(JSON.stringify({base,cookie:cookie(a),connection:C}));const [exit]=await once(childBrowser,'exit');console.log(output);assert.equal(exit,0,'BROWSER_FLOW');
