...
export async function launch({services=false}={}) {
  ...
  const allocated=[];for(let p=57920;p<=57949&&allocated.length<3;p++){const server=createServer();try{server.listen(p,'127.0.0.1');await once(server,'listening');allocated.push(p);}catch{}finally{if(server.listening)await new Promise(r=>server.close(r));}}
  if(allocated.length<3)throw Error('NO_FREE_PORTS');
  const prefix='vexa-f01-03-'+randomUUID(), owned=[];
  const credentials=new Map();const ports={};
  let network=false;
  ...
  const run=(kind,env={},port)=>{
    ...
    docker(['run','--pull','never','-d','--name',name,...resources.reserve('container',name),'--network',prefix,
      ...(port?['-p',`127.0.0.1:${allocated.shift()}:${kind==='storage'?5000:kind==='auth'?9999:3000}`]:[]),
      ...Object.entries(env).flatMap(([k,v])=>['-e',`${k}=${v}`]),images[kind]]);
    if(port)ports[kind]=Number(docker(['port',name,`${kind==='storage'?5000:kind==='auth'?9999:3000}/tcp`]).split(':').at(-1));
  };
  try {
