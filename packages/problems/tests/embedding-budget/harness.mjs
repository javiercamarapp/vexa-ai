...
export async function setup(candidate,evidence){
 for(const f of ['packages/workspace-service/detail.mjs'])assert.ok(candidate&&fs.existsSync(path.join(candidate,f)),'IMPLEMENTATION_MISSING:'+f);
 const driver=fs.mkdtempSync(path.join(os.tmpdir(),'embedding-budget325-driver-'));let h;const pools=[];
 try{let code=fs.readFileSync(pathToFileURL(path.join(candidate,'tests/acceptance/support/F02-durable-final/harness.mjs')),'utf8');
 ...
 for(let n=0;n<6;n++)code=code.replaceAll(String(58160+n),String(62820+n));
 code=code.replace("'packages/jobs','packages/ingestion'","'packages/workspace-service','packages/metrics','packages/economics','packages/problems','packages/gateway','packages/intelligence','packages/connectors','packages/jobs','packages/ingestion'");
 code=code.replace('h.built=built;h.common=common;','h.built=built;h.common=common;fs.cpSync(path.join(tmp,\'packages/connectors\'),path.join(built,\'packages/connectors\'),{recursive:true});for(const module of [\'workspace-service\',\'metrics\',\'economics\',\'problems\',\'gateway\',\'intelligence\'])fs.cpSync(path.join(tmp,\'packages/\'+module),path.join(built,\'packages/\'+module),{recursive:true});');
