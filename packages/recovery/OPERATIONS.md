...
El ensayo352 amplía la evidencia local hasta SQL0038, con historiales de evaluación, selección/rollback y equipos no vacíos. No es restauración de Auth/Storage gestionados; el destino continúa cerrado y deben reconciliarse las revocaciones posteriores al backup antes de abrir acceso. Quedan además validación remota, política/custodia real y autorización de producción. El CLI de ensayo sigue siendo exclusivamente local/sintético. Los apartados de correcciones que siguen conservan sus cortes históricos; frases como “propuesta no publicada” o “API pendiente” describen esos cortes, no este estado integrado. Migraciones experimentales diferentes ya aplicadas requieren expansión revisada, nunca reescribir una migración instalada.
...
## Ensayo local del esquema hasta SQL0038 — ensayo352 revisado

El ensayo sobre `70e61a2d988700b9f4995f315bd0d94786af2878` aplica todas las migraciones hasta 0038 en dos bases PostgreSQL 17 separadas. `backupLocal` produce el dump y copia objetos sintéticos; `restoreLocal` verifica el mismo esquema, restaura y reaplica los ledgers de borrado de ambos tenants. El backup original se conserva y su hash no cambia. Las guardas de conexión permanecen cerradas al finalizar.

Resultado: 5/5 comprobaciones en Node 26.7.0, salida 0, sin señal ni error de proceso. Se conservan byte-equivalentes las filas de cohortes, ítems, feedback, resultado candidato, invitaciones y auditoría de equipos. Owner mantiene lectura; viewer y owner de otro tenant no acceden. Repetir una invitación ya aceptada por un miembro revocado o aceptar una expirada se rechaza. La extracción borrada permanece fuera del export de desarrollo, rechaza feedback nuevo y conserva coste desconocido como `null`. Textos derivados, raw, vectores y objetos registrados siguen purgados; el snapshot financiero conserva su hash.

RPO observado: 604 ms entre marcadores sintéticos; RTO observado por el restaurador: 707 ms. Son observaciones locales, no SLA. El rollback comprobado es únicamente la consulta SQL financiera anterior sobre el esquema expandido, no el binario web anterior.

Límites: Auth usa bootstrap SQL sintético con los campos requeridos por 0038, no HTTP Auth ni una copia de Auth gestionado. Storage usa el adaptador de archivos local real. El primer resultado candidato es explícitamente no evaluado y conserva coste desconocido. Un segundo usa la fixture contractual SYN de la revisión 344, con firma Ed25519 efímera y metadatos fabricados expresamente para la rama de elegibilidad; no representa gold humano real. Selección versión 1 y rollback versión 2 conservan sus filas y hashes tras restore. El repositorio comprueba elegibilidad, rechaza CAS antiguo, ejecuta rollback a la selección previa como versión 3 y rechaza la clave custodio revocada. No hubo inferencia ni evaluación humana. No se acredita recuperación de revocaciones de membresía posteriores al backup: el ledger de retención sólo cubre sus borrados. Continúan pendientes el ensayo del destino gestionado, la configuración operacional y la aprobación de release.

Resumen público: [recovery-results.json](../../docs/blueprint/recovery-results.json). Los recibos privados y el rojo del bootstrap mínimo anterior se conservan junto al verde. El principal comprobó fuentes ejecutables, ocho archivos del backup, el índice del dump y las siete tablas restauradas; revisó los controles sin atribuirse una segunda corrida completa. La corrección sólo adapta el harness, sin cambios al producto ni a migraciones. Este ensayo amplía la evidencia local anterior de SQL0035 y no la presenta como equivalente por sí sola a SQL0038.
