...
export async function setup(candidate,evidence){
 ...
 const driver=fs.mkdtempSync(path.join(os.tmpdir(),'vexa-retention335-driver-'));let h;const pools=[];
 try{
  ...
  for(let n=0;n<6;n++)code=code.replaceAll(String(58160+n),String(63520+n));
  code=code.replace("'packages/jobs','packages/ingestion'","'packages/recovery','packages/gateway','packages/intelligence','packages/notifications','packages/briefs','packages/interventions','packages/recommendations','packages/workspace-service','packages/metrics','packages/economics','packages/problems','packages/connectors','packages/jobs','packages/ingestion'");
  // Reuse complete source hashes. Runtime methods use actual product modules, never behavioral replacements.
  code=code.replace('h.built=built;h.common=common;',`h.built=built;h.common=common;for(const name of ['recovery','gateway','intelligence','notifications','briefs','interventions','recommendations','workspace-service','metrics','economics','problems','connectors'])fs.cpSync(path.join(tmp,'packages',name),path.join(built,'packages',name),{recursive:true});`);
