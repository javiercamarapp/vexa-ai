...
add('F07-01','docs/blueprint/security-findings.json|packages/platform/src/authorization.ts',
 'Ejecutar SEC-01..08 sobre DB, Storage, vectores, cache, exports y jobs locales reales, con usuarios A/B e IDs conocidos.|Atacar endpoints directamente; revisar exposición de service_role y SSRF mediante adjuntos.|Revocar acceso durante un job y repetir descarga; inspeccionar logs por PII y tokens.|Registrar exploit y reproducción concretos; habilitar RLS no demuestra aislamiento.',
...
add('F07-06','docs/blueprint/restore-drill.json|packages/jobs/src/deletion.ts|supabase/migrations/0015_tombstones.sql',
