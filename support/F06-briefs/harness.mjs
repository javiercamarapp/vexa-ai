...
export async function setup(candidate,evidence){
 for(const f of ['packages/briefs/index.mjs'])assert.ok(candidate&&fs.existsSync(path.join(candidate,f)),'IMPLEMENTATION_MISSING:'+f);
 ...
 code=code.replace("'packages/jobs','packages/ingestion'","'packages/briefs','packages/interventions','packages/recommendations','packages/workspace-service','packages/metrics','packages/economics','packages/problems','packages/gateway','packages/intelligence','packages/connectors','packages/jobs','packages/ingestion'");
 code=code.replace('h.built=built;h.common=common;','h.built=built;h.common=common;fs.cpSync(path.join(tmp,\'packages/connectors\'),path.join(built,\'packages/connectors\'),{recursive:true});for(const module of [\'briefs\',\'interventions\',\'recommendations\',\'workspace-service\',\'metrics\',\'economics\',\'problems\',\'gateway\',\'intelligence\'])fs.cpSync(path.join(tmp,\'packages/\'+module),path.join(built,\'packages/\'+module),{recursive:true});');
