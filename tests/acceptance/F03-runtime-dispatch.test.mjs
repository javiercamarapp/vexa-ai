...
import {launch,candidateInputs} from './support/F01-03/harness.mjs';
...
test('hosted consumer rotation is durable, independent and authorized', {timeout:180000},async t=>{
 ...
 try{
  ...
  const reset=()=>h.sql('UPDATE worker_delegations SET last_dispatched_at=null,crm_last_dispatched_at=null,extraction_last_dispatched_at=null');
  ...
  await t.test('three interleaved consumers reach both authorized tenants and never foreign scope',()=>{
   fs.writeFileSync(path.join(evidence,'rotation.json'),JSON.stringify({twoConsumers:fairness(),threeConsumers:fairness(['imports','crm','extraction'])},null,2),{mode:0o600});
   assert.equal(h.sql(`SELECT last_dispatched_at IS NULL AND crm_last_dispatched_at IS NULL AND extraction_last_dispatched_at IS NULL FROM worker_delegations WHERE tenant_id=${q(C)}`),'t','FOREIGN_DISPATCH_UNCHANGED');
  });
  ...
  await t.test('consumer allowlist, callable roles, current identity and action fail closed',()=>{
   ...
   for(const kind of ['imports','crm','extraction']){
  ...
  await t.test('revoked membership, disabled delegation and changed role stop each consumer',()=>{
   for(const kind of ['imports','crm','extraction'])for(const prep of [
