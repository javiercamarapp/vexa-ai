...
function docker(args,input) {
  ...
  assert.ok(!r.error && r.status===0,`INFRA_F01_04: docker ${args[0]} failed (${r.status??r.error?.code}); no authorization verdict; ${r.stderr?.match(/(?:permission denied|must be|does not exist)[^\n]*/)?.[0]??'details suppressed'}`);
...
export async function launch({services=false}={}) {
  ...
  const prefix='vexa-f01-04-'+randomUUID(), owned=[];
  ...
  const h={anon,service,
    ...
    async http(kind,route,token,{method='GET',body,headers={}}={}) {
      const base=`http://127.0.0.1:${{auth:57561,storage:57562,rest:57563}[kind]}`;
    ...
    async user(){
      ...
      credentials.set(r.data.user.id,body);return {id:r.data.user.id,token:r.data.access_token,session:r.data};
    },
    async login(actor){
  ...
  const run=(kind,env={},port)=>{
    ...
    docker(['run','--pull','never','-d','--name',name,...resources.reserve('container',name),'--network',prefix,
      '--label','vexa.exam=F01-04',...(port?['-p',`127.0.0.1:${port}:${kind==='storage'?5000:kind==='auth'?9999:3000}`]:[]),
  ...
  try {
    ...
    if(services){
      run('auth',{GOTRUE_API_HOST:'0.0.0.0',GOTRUE_API_PORT:9999,API_EXTERNAL_URL:'http://127.0.0.1:57561',GOTRUE_SITE_URL:'http://127.0.0.1:57561',GOTRUE_DB_DRIVER:'postgres',GOTRUE_DB_DATABASE_URL:`postgres://supabase_auth_admin:${password}@${prefix}-db:5432/postgres`,GOTRUE_JWT_SECRET:secret,GOTRUE_JWT_AUD:'authenticated',GOTRUE_JWT_DEFAULT_GROUP_NAME:'authenticated',GOTRUE_DISABLE_SIGNUP:false,GOTRUE_MAILER_AUTOCONFIRM:true},57561);
      run('rest',{PGRST_DB_URI:`postgres://authenticator:${password}@${prefix}-db:5432/postgres`,PGRST_DB_SCHEMAS:'public',PGRST_DB_ANON_ROLE:'anon',PGRST_JWT_SECRET:secret},57563);
