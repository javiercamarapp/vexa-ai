...
export async function launch({services=false}={}) {
  ...
  const h={
    productConfiguration(){
      const login='f02_product',pass='SYN-'+randomBytes(24).toString('hex');
      h.sql(`CREATE ROLE ${login} LOGIN NOINHERIT NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE PASSWORD ${q(pass)}; GRANT vexa_backend TO ${login};`);
      return {container:prefix+'-storage',connection:{host:prefix+'-db',database:'postgres',user:login,password:pass},anon};
    },
    sqlAsync(statement){return new Promise((resolve,reject)=>{const child=execFile('docker',['exec','-i',prefix+'-db','psql','-X','-qAt','-U','supabase_admin','-d','postgres','-v','ON_ERROR_STOP=1'],{timeout:15000,maxBuffer:1024*1024},(error,stdout,stderr)=>error?reject(Object.assign(new Error('SQL_ASYNC_FAILED'),{sqlstate:stderr.includes('duplicate key')?'23505':null})):resolve(stdout.trim()));child.stdin.end(statement);});},
    signStorageClockToken(token,expired){const claims=JSON.parse(Buffer.from(token.split('.')[1],'base64url'));claims.exp=Math.floor(Date.now()/1000)+(expired?-60:300);const raw=[{alg:'HS256',typ:'JWT'},claims].map(x=>Buffer.from(JSON.stringify(x)).toString('base64url')).join('.');return raw+'.'+createHmac('sha256',secret).update(raw).digest('base64url');},
  ...
  try {
    ...
    assert.equal(docker(['inspect',prefix+'-db','--format','{{json .HostConfig.PortBindings}}']),'{}','INFRA_DB_NO_HOST_PORTS');
