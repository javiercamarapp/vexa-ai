...
export async function runtime(candidate){
 ...
 try{
  ...
  let clock=Date.now();
  const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'f02-db-inputs-'));
  for(const name of ['session','db']){
   const source=fs.readFileSync(path.join(candidate,'packages/platform/src/'+name+'.ts'),'utf8');
   fs.writeFileSync(path.join(tmp,name+'.mjs'),name==='session'?`export * from ${JSON.stringify(pathToFileURL(path.join(candidate,'packages/platform/src/session.ts')).href)};`:stripTypeScriptTypes(source).replaceAll("'@vexa/platform/session'","'./session.mjs'"));
  }
  const config={...h.productConfiguration(),compiled:tmp,clock,signingSecret:randomUUID()};
  const ports=await canonicalPorts(config),close=h.close.bind(h);
  h.close=()=>{ports.close();try{close();}finally{fs.rmSync(tmp,{recursive:true,force:true});}};
  return Object.assign(h,{A,B,ports,pool:ports.pool,processConfig:config,advance:ms=>{clock+=ms;config.clock=clock;}});
...
export function injection(h,table,on){
 ...
 h.sql(on?`CREATE SEQUENCE public.f02_fault_reached START 1; CREATE OR REPLACE FUNCTION public.f02_exam_fault() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN PERFORM nextval('public.f02_fault_reached'); RAISE EXCEPTION 'SYNTHETIC_F02_FAULT' USING ERRCODE='P0001'; END $$; GRANT USAGE, SELECT ON SEQUENCE public.f02_fault_reached TO vexa_backend; CREATE TRIGGER f02_exam_fault BEFORE INSERT ON public.${table} FOR EACH ROW EXECUTE FUNCTION public.f02_exam_fault();`:`DROP TRIGGER f02_exam_fault ON public.${table}; DROP FUNCTION public.f02_exam_fault(); DROP SEQUENCE public.f02_fault_reached;`);
}
