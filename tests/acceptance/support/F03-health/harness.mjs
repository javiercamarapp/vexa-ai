...
export async function setup(candidate,evidence){
 for(const f of ['packages/connectors/health.mjs','supabase/migrations/0010_connection_health.sql','apps/web/src/components/connection-health.tsx'])assert.ok(candidate&&fs.existsSync(path.join(candidate,f)),'IMPLEMENTATION_MISSING:'+f);
 const driver=fs.mkdtempSync(path.join(os.tmpdir(),'f0305-driver-'));let h;const pools=[];
 try{let code=fs.readFileSync(new URL('../F02-durable-final/harness.mjs',import.meta.url),'utf8');
 const base=new URL('../F02-durable-final/harness.mjs',import.meta.url);
 ...
 for(let n=0;n<6;n++)code=code.replaceAll(String(58160+n),String(58520+n));
 code=code.replace("'packages/jobs','packages/ingestion'","'packages/connectors','packages/jobs','packages/ingestion'");
 code=code.replace('h.built=built;h.common=common;','h.built=built;h.common=common;fs.cpSync(path.join(tmp,\'packages/connectors\'),path.join(built,\'packages/connectors\'),{recursive:true});');
